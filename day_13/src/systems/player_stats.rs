...
pub fn update_player_stats_panel(
    player_query: Query<(&Attack, &Defense), With<Player>>,
    mut text_query: Query<&mut Text, With<PlayerStatsText>>,
) {
    let Some((attack, defense)) = player_query.iter().next() else {
        return;
    };
    ...
    let content = format!(
        "{}\n{}\n{}\n{}\n{}",
        header_line, attack_line, defense_line, hint_line_top, hint_line_bottom
    );
