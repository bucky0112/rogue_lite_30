    ...
    if keyboard_input.just_pressed(KeyCode::Space) {
        ...
        for (exit, exit_transform) in &exit_query {
            ...
            if distance <= LEVEL_EXIT_INTERACTION_RADIUS {
                ...
                dev_info!(
                    "🔁 Advancing to level {} (distance {:.1}, player {:?}, exit {:?})",
                    exit.target_level + 1,
                    distance,
                    player_position,
                    exit_position
                ...
                dev_info!(
                    "🚶 Next exit is {:.1} units away (needs {:.1}); player {:?}, exit {:?}",
                    distance,
                    LEVEL_EXIT_INTERACTION_RADIUS,
                    player_position,
                    exit_position
                );
            }
        ...
        if near_door {
            ...
            dev_info!("Door interaction event dispatched");
        } else {
