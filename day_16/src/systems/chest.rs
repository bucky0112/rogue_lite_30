...
use crate::constants::*;
use crate::systems::items::{pickup_visual_for_effect, random_pickup_effect};
use bevy::prelude::*;
use rand::prelude::*;
...
pub fn spawn_random_chests(
    ...
    let spawn_count = CHEST_SPAWN_COUNT.min(positions.len());
    for base_position in positions.into_iter().take(spawn_count) {
        let contents = if rng.gen_range(0.0..1.0) < CHEST_MIMIC_PROBABILITY {
            ChestContents::Mimic
        } else {
            ChestContents::Item(random_pickup_effect(&mut rng))
        };
