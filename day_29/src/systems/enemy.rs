...
pub fn despawn_dead_enemies_system(
    ...
    ) in &mut query
    {
        ...
        info!("Enemy defeated, dropped {} and started fading", drop_label);
    }
}
...
pub fn enemy_death_effect_system(
    ...
    for (entity, mut effect, mut sprite) in &mut query {
        ...
        if effect.timer.finished() {
            info!("Enemy fade-out finished; entity removed");
            commands.entity(entity).despawn();
        }
    }
}
...
pub fn reset_enemies_on_player_respawn(
    ...
    info!("Enemy roster reset to initial state");
}
