...
pub fn enemy_contact_attack_system(
    ...
    mut attacker_query: Query<
        ...
        (Without<EnemyDeathEffect>, Or<(With<Slime>, With<Mimic>)>),
    >,
...
pub fn despawn_dead_enemies_system(
    ...
    for (entity, transform, health, death_effect, mut sprite, slime, cyclops, mimic) in &mut query {
        if health.current > 0 {
            continue;
        }
        ...
        let (experience_reward, enemy_label) = if slime.is_some() {
