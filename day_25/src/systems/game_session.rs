    ...
    match serde_json::to_string_pretty(&data) {
        Ok(serialized) => {
            if let Err(error) = fs::write(GameSession::SAVE_SLOT_FILE, serialized) {
                ...
                info!("遊戲進度已儲存至 {}", GameSession::SAVE_SLOT_FILE);
            }
        }
        Err(error) => {
            error!("序列化存檔資料失敗：{error}");
        }
    }
}
        ...
        .with_children(|parent| {
            parent
                ...
                .with_children(|panel| {
                    ...
                    let actions = [
                        (PauseMenuAction::Resume, "返回遊戲"),
                        (PauseMenuAction::Save, "儲存進度"),
                        (PauseMenuAction::Load, "讀取進度"),
                    ];
